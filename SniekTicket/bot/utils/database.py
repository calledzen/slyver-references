...
def tableSetup():
    cur.execute("CREATE TABLE IF NOT EXISTS `ticketsettings` (`ticketname` VARCHAR(150), `logchannel` VARCHAR(25), `createchannel` VARCHAR(25), `teamroleid` VARCHAR(25));")
    cur.execute("CREATE TABLE IF NOT EXISTS `moddata` (`userid` VARCHAR(25), `ticketsdone` INT(7) NOT NULL DEFAULT '0', `lastticketid` INT(7));")
    cur.execute("CREATE TABLE IF NOT EXISTS `tickets` (`ticketid` INT(7), `ownerid` VARCHAR(25), `channelid` VARCHAR(25), `timestamp` VARCHAR(50), `modid` VARCHAR(25), `state` VARCHAR(25), `concern` TEXT(4000), `detailedconcern` TEXT(4000), `ticketform` VARCHAR(150));")
    con.commit()
