...
def tableSetup():
    cur.execute("CREATE TABLE IF NOT EXISTS `ticketsettings` (`ticketname` VARCHAR(150), `logchannel` VARCHAR(25), `createchannel` VARCHAR(25));")
    cur.execute("CREATE TABLE IF NOT EXISTS `moddata` (`userid` INT(25), `ticketsdone` INT(7), `lastticketid` INT(7));")
    cur.execute("CREATE TABLE IF NOT EXISTS `tickets` (`ticketid` INT(7), `ownerid` INT(20), `channelid` INT(20), `timestamp` VARCHAR(50), `modid` INT(25), `state` VARCHAR(25), `concern` TEXT(4000), `detailedconcern` TEXT(4000), `ticketform` VARCHAR(150));")
    con.commit()
